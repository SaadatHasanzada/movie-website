...
const Bookmark:React.FC<StyleBookmark> = ({className,title,isBookmarked}) => {
  const { toggleBookmark } = useBookmarkContext();
const customStyle=isBookmarked ? style.bookmarked : '';
  return (
    <>
    <button   onClick={()=>{toggleBookmark(title)}}  className={`${style.bookmark} ${className ? style.trendingBookmark : ''} ${customStyle}`}>
